...
class _SubcategoryWindowState extends State<SubcategoryWindow> {
  ...
  Widget build(BuildContext context) {
    final subcategories =
        context.select<CatalogueBloc, List<CatalogueSubcategoriesModel>>(
      (bloc) => bloc.state.catalogueSubcategories,
    );
    final isLoading = context.select<CatalogueBloc, bool>(
      (bloc) => bloc.state.isLoadingSubcategories,
    );
    final errorMessage = context.select<CatalogueBloc, String>(
      (bloc) => bloc.state.subcategoriesErrorMessage,
    );
          ...
          Padding(
            ...
            child: Center(
              child: Text(
                widget.categoryName,
                style:
                    CataloguePageTextStyles.subcategoriesCategoryNameTextStyle,
              ),
            ),
          ),
          Expanded(
